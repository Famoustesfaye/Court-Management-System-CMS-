const FetchCasesByJudge = async (db, req, res) => {
  try {
    console.log("FetchCasesByJudge called");
    
    // Check for assignedJudgeId in multiple possible locations
    const assignedJudgeId = req.body?.assignedJudgeId || req.query?.assignedJudgeId || req.params?.assignedJudgeId;
    
    console.log("Request body:", req.body);
    console.log("Request query:", req.query);
    console.log("Judge ID received:", assignedJudgeId);

    if (!assignedJudgeId) {
      return res.status(400).json({ 
        error: "Judge ID is required",
        message: "Please provide assignedJudgeId in request body, query parameters, or URL parameters"
      });
    }
...
`;
    ...
    results.forEach(row => {
      ...
      if (!groupedResults[caseId]) {
        groupedResults[caseId] = {
          ...
          sub_type_case_type: row.sub_type_case_type,
    ...
    console.log(`Found ${finalResults.length} cases for judge ${assignedJudgeId}`);
